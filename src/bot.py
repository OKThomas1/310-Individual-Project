...
class Bot:
	...
	def initialize(self, filePath):
		...
		self.language = os.environ['LANG'][:2]
		self.language = "es"
		self.wikipedia = Wikipedia(self.language)

		for key in self.data['conditions'].keys():
			if self.language != "en":
				text = " ".join(self.data['conditions'][key])
				thing = self.translator.translate(text, dest=self.language)
				words = [self.stemmer.stem(word.lower()) for word in thing.text.split(" ")]
			else:
	...
	def handleWikiAnswer(self, node, answer):
		self.current = self.findNode(node['children'][0])
		tags = pos_tag(answer)
		best = ""
		for word, pos in tags:
			if "NN" in pos and len(word) > len(best):
				best = word
		if best == "":
			return {"text": "Sorry, I dont understand."}
		page = self.wikipedia.page(best)
		if page.exists():
			return {"text": f"Here's what I found about {best}:\n {page.summary}"}
		return {"text": f"I could not find any information on {best}, sorry."}
	...
	def getResponse(self, answer):
		...
		if len(nodeValue['children']) == 1:
			...
			for child in nodeValue['children']:
				...
				if 'default' in child:
					if 'wiki' in child:
						return self.handleWikiAnswer(child, answer.split(" "))
					nodeValue = child
					break
