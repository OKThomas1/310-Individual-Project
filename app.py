...
class GUI:
    ...
    def initializeBot(self):
        ...
        if self.bot.language != "en":
            msg2 = self.bot.translator.translate(msg2, dest=self.bot.language).text + "\n\n"
    ...
    def addMessage(self, msg):
        ...
        if self.bot.getUserName() == -1:
            ...
            name_message = self.bot.setUserName(msg)
            if self.bot.language != "en":
                name_message = self.bot.translator.translate(name_message, dest=self.bot.language).text + "\n\n"
            self.renderMessage(name_message)
        else:
            # if no more nodes in dialogue or user printed "quit" - exit the program
            try:
                response = self.bot.getResponse(msg)['text']
                if self.bot.language != "en":
                    response = self.bot.translator.translate(response, dest=self.bot.language).text + "\n\n"
            except:
                sys.exit()
